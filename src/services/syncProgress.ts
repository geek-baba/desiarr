...
interface SyncProgress {
  isRunning: boolean;
  type: 'radarr' | 'rss' | 'matching' | 'full' | 'sonarr' | 'tv-matching' | 'tmdb-sync' | 'tmdb-backfill';
  currentStep: string;
...
export const syncProgress = {
  start: (type: 'radarr' | 'rss' | 'matching' | 'full' | 'sonarr' | 'tv-matching' | 'tmdb-sync' | 'tmdb-backfill', total: number = 0) => {
    currentProgress = {
      isRunning: true,
      type,
  ...
  error: (message: string) => {
    if (currentProgress) {
      currentProgress.currentStep = message;
      currentProgress.isRunning = false;
      currentProgress.errors = currentProgress.errors + 1;
      currentProgress.endTime = new Date();
    }
  },
};
